...
------WebKitFormBoundaryDf9uRfwb8uzv1eNe--
    ''' % (os.path.basename(dataFile), data)
        ...
        status, reason = interfaceTest(num, api_purpose, api_host, request_url, request_data, check_point,
                                     request_method, request_data_type, correlationDict['${session}'])

        errorCase.append((num + ' ' + api_purpose, str(status), api_host + request_url, reason))
        # if status != 200:
        #     errorCase.append((num + ' ' + api_purpose, str(status), api_host + request_url, reason))
        #     continue
...
def interfaceTest(num,api_purpose,api_host,request_url,request_data,check_point,request_method,request_data_type,session):  
    ...
    print(api_host + request_url)
    print(request_data)
    resp = requests.post(api_host + request_url, json=request_data, headers=headers)
    status = resp.status_code
    reason = resp.reason

    if status == 200:
        # resp = resp.decode('utf-8')
        logging.info(num + ', ' + api_purpose + ', ' + str(status) + ', ' + str(reason))
        # if re.search(check_point, str(resp)):
        #     logging.info(num + ', ' + api_purpose + ', ' + str(status) + ', ' + str(resp))
        #     return status, json.loads(resp)
        # else:
        #     logging.error(num + ' ' + api_purpose + ', ' + str(status) + ', ' + str(resp))
        #     return 2001, resp
    else:
        logging.error(num + ' ' + api_purpose + ', ' + str(status) + ', ' + str(reason))
